...
bpf_table_fd_id, bpf_lookup_elem, bpf_table_leaf_snprintf, bpf_get_next_key,
bpf_get_first_key, bpf_table_key_snprintf};
use std::ffi::CString;
...
impl<'a> IntoIterator for &'a Table<'a> {
    ...
    fn into_iter(self) -> Self::IntoIter {
        ...
        let fd = unsafe {
            bpf_table_fd_id(p, self.id)
        };
        let key_size = unsafe {
            bpf_table_key_size_id(p, self.id)
        };
        let leaf_size = unsafe {
            bpf_table_leaf_size_id(p, self.id)
        };
        TableIterator {
            table: self,
            key: vec![0; key_size],
...
impl<'a> Iterator for TableIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if !self.init {
            r = unsafe {
                bpf_get_first_key(self.fd, self.key.as_ptr() as *mut _, self.key.len()) 
            };
            if r < 0 {
                return None
            }
            ...
            r = unsafe {
                bpf_get_next_key(self.fd, self.key.as_ptr() as *mut _, self.key.as_ptr() as *mut _)
            };
            if r != 0 {
                return None;
            }
        ...
        r = unsafe {
            bpf_lookup_elem(self.fd, self.key.as_ptr() as *mut _, leaf.as_ptr() as *mut _)
        };
        if r != 0 {
            return None;
        }
        r = unsafe {
            bpf_table_key_snprintf(p, self.table.id, key_str.as_ptr() as *mut _,
            key_str.len(), self.key.as_ptr() as *mut _)
        };
        if r != 0 {
            return None;
        }
        r = unsafe {
            bpf_table_leaf_snprintf(p, self.table.id, leaf_str.as_ptr() as *mut _,
            leaf_str.len(), leaf.as_ptr() as *mut _)
        };
        if r != 0 {
            return None;
        }
